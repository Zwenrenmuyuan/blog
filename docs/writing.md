...
- 文章封面会同时用作该文章的社交分享图；没有封面时使用站点默认分享图。
...
- 公开文章的标题、摘要、中文标签和正文会进入 Pagefind 搜索索引；页面导航、目录和推荐卡片不会重复入库。
- 搜索索引只在生产构建后存在。本地验证时先运行 `npm run build`，再运行 `npm run preview` 并访问 `/search/`。
- RSS 位于 `/rss.xml`，包含最近二十篇公开文章的摘要、日期和标签。
- 公开文章会进入 canonical、结构化数据和 sitemap；草稿不会进入搜索、RSS、sitemap 或结构化数据，并带有禁止收录标记。
- 标题、摘要、发布日期、更新日期、标签和封面也用于 SEO 与社交分享，发布前需要逐项核对。
